...
- [ ] Read `src/apps/RsDorDcApp/services/searchParserService.js` end-to-end
- [ ] Read `src/apps/RsDorDcApp/index.jsx` — trace how `parsedQuery` / `parsedQueryRef` is used in `customQuery`
- [ ] Document the current flow: raw input → parser service → customQuery
- [ ] Identify all code paths that would be affected by switching to DSL
...
- [ ] Decide on a test framework / approach for the React side (Jest + React Testing Library)
- [ ] Write unit tests for `searchParserService.js` covering the `parsed_query_dsl` field
- [ ] Write tests for the `customQuery` callback in `RsDorDcApp/index.jsx` using DSL input
- [ ] Write integration/smoke tests that verify the end-to-end search path (mock service)
- [ ] Verify the current state of the project achieves the task goal
- [ ] Verify with the developer that the task is complete
...
- [ ] Update `searchParserService.js` to extract and expose `parsed_query_dsl`
- [ ] Update `customQuery` callback in `RsDorDcApp/index.jsx` to use DSL object directly
- [ ] Preserve backward compatibility fallback (raw query) when parser unavailable
- [ ] Run `npm run lint | cat` and fix any lint errors
